...
refactored by Aviad Rozenhek to add a hook for custom rules
"""
...
def apply_default_rules(grid, i, j, total_neighbours):
...
def apply_rules(grid, i, j, total_neighbours):
    global custom_rules
    if not custom_rules:
        return apply_default_rules(grid, i, j, total_neighbours)
    else:
        return apply_custom_rules(custom_rules, grid, i, j, total_neighbours)
...
def update(frameNum, img, grid, N):
    ...
    for i in range(N):
        for j in range(N):
            ...
            newGrid[i, j] = apply_rules(grid, i, j, total)
...
if __name__ == '__main__':
    read_custom_rules(custom_rules)
    main()